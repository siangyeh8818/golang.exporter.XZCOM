...
func CallSelium() {
	...
	for {
		balance := RunSelium()
		tool.WriteWithIoutil("/root/output.csv", balance)
		internal_time, _ := strconv.Atoi(os.Getenv("SELEIUM_INTERNAL_TIME"))
		time.Sleep(time.Duration(internal_time) * time.Second)
	}
}

func RunSelium() float64 {

	const (
		seleniumPath = `/usr/local/share/chromedriver`
		//port         = 9515
	)
